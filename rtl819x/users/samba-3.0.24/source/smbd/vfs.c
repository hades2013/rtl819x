...
int vfs_set_filelen(files_struct *fsp, SMB_OFF_T len)
{
	...
	flush_write_cache(fsp, SIZECHANGE_FLUSH);
//	printf("\nbefore SMB_VFS_FTRUNCATE\n");
	ret = SMB_VFS_FTRUNCATE(fsp, fsp->fh->fd, len);
//	printf("\nafter SMB_VFS_FTRUNCATE\n");
	if (ret != -1)
		set_filelen_write_cache(fsp, len);
...
int vfs_set_filelen_1(files_struct *fsp, SMB_OFF_T len)
{
	int ret;
	...
	flush_write_cache(fsp, SIZECHANGE_FLUSH);
//	printf("\nbefore SMB_VFS_FTRUNCATE\n");
	ret = SMB_VFS_FTRUNCATE(fsp, fsp->fh->fd, 0);
//	printf("\nafter SMB_VFS_FTRUNCATE\n");
	if (ret != -1)
		set_filelen_write_cache(fsp, len);
