...
#if defined( HTTP_FILE_SERVER_SUPPORTED)
	ret=1;
	setsockopt(sp->sock, IPPROTO_TCP, TCP_NODELAY, (char *)&ret, sizeof(ret));
	
	opt = priority << 5;    
	ret = setsockopt(sp->sock, SOL_IP, IP_TOS, &opt, sizeof(opt));
	if (ret < 0)		
		printf("%s:setsockopt IP_TOS error!!\n",__FUNCTION__);
	opt = priority;    
	setsockopt(sp->sock, SOL_SOCKET, SO_PRIORITY, &opt, sizeof(opt)); 
	if (ret < 0)		
		printf("%s:setsockopt SO_PRIORITY error!!\n",__FUNCTION__);
#endif	
...
#if defined(HTTP_FILE_SERVER_SUPPORTED)
 		rc=1;
		if(setsockopt(sp->sock, IPPROTO_TCP, TCP_NODELAY, (char *)&rc, sizeof(rc))<0){
			printf("%s:set TCP_NODELAY error\n", __FUNCTION__);
		}
		opt = priority << 5;	
		ret = setsockopt(sp->sock, SOL_IP, IP_TOS, &opt, sizeof(opt));
		if (ret < 0)		
			printf("%s:setsockopt IP_TOS error!!\n",__FUNCTION__);
		opt = priority;    
		ret=setsockopt(sp->sock, SOL_SOCKET, SO_PRIORITY, &opt, sizeof(opt)); 
		if (ret < 0)		
			printf("%s:setsockopt SO_PRIORITY error!!\n",__FUNCTION__);
#endif
