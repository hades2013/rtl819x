 ...
 *
 * $Id: default.c,v 1.2 2009/06/16 01:40:45 keith_huang Exp $
 */
...
extern void put_time_buf(char *buf, time_t s);
extern void websWriteFileDone(wp);
#endif
#if defined(HTTP_FILE_SERVER_SUPPORTED)
void websDefaultWriteEvent(webs_t wp);
#else
 ...
 */
	...
	if(!(flags & WEBS_ASP)){
		websWrite(wp, T("Accept-Ranges: bytes\r\n"));
	}
	if (bytes) {
	...
	if(strstr(url,".cer") != 0)
	{
		websWrite(wp, T("Content-Disposition: attachment\r\n"));
	}
#endif
