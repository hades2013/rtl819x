...
<table width=400 border=0">
  ...
  var mssid_wep=new Array(wlan_num);
	...
	for (i=0; i<getIndex("wlan_num"); i=i+1)
	{
		...
		{
			write("mssid_ssid_drv["+i+"]["+k+"] ='");getVirtualInfo("ssid_drv", k+1);write("';\n");			
			write("mssid_band["+i+"]["+k+"] ="+getVirtualIndex("band", k+1)+";\n");
			write("mssid_disable["+i+"]["+k+"] ="+getVirtualIndex("wlanDisabled", k+1)+";\n");			
			write("mssid_bssid_drv["+i+"]["+k+"] ='");getVirtualInfo("bssid_drv", k+1);write("';\n");			
			write("mssid_clientnum["+i+"]["+k+"] ='");getVirtualInfo("clientnum", k+1);write("';\n");
			write("mssid_wep["+i+"]["+k+"] ='");getVirtualInfo("wep", k+1);write("';\n");
		}	
	}
	%>
    ...
    for(i=0; i < wlan_num ; i++)
   	{
   		if(ssid_drv[i]=="")
	    		...
	    		mssid_num=<%write(getIndex("wlan_mssid_num"));%>;
		...
		for (idx=0; idx<mssid_num; idx++) {
			if (!mssid_disable[i][idx]) {
				...
				<tr bgcolor="#DDDDDD">\
			    	<td width=40%><font size=2><b>SSID</b></td>\
			    	<td width=60%><font size=2>');
				document.write('<pre><font size=2>'+mssid_ssid_drv[i][idx]+'</td>');
