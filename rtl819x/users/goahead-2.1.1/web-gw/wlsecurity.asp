...
function ValidateForm(passForm, reset)
{
	...
	if (reset) {
		...
		if (self.SSIDAuthMode.ValidateForm(passForm) == true) {
			 if ((passForm.method<% write(getIndex("wlan_idx")); %>.value == 7) && (passForm.wapiAuth<% write(getIndex("wlan_idx")); %>[0].checked)){
			 	// For wapi-cert only
...
<table>
<input name="wapiAuth<% write(getIndex("wlan_idx")); %>" type="radio" value="eap" >PSK
<input name="wapiAuth<% write(getIndex("wlan_idx")); %>" type="radio" value="psk">Pre-Shared&nbsp;Key
</table>
</span>
...
<input type="hidden" id="eap_type" name="eapType<% write(getIndex("wlan_idx")); %>" value="">
<input type="hidden" id="eap_inside_type" name="eapInsideType<% write(getIndex("wlan_idx")); %>" value="">
<input type="hidden" id="eap_user_id" name="eapUserId<% write(getIndex("wlan_idx")); %>" value="">
<input type="hidden" id="radius_user_name" name="radiusUserName<% write(getIndex("wlan_idx")); %>" value="">
<input type="hidden" id="radius_user_pass" name="radiusUserPass<% write(getIndex("wlan_idx")); %>" value="">
<input type="hidden" id="radius_user_cert_pass" name="radiusUserCertPass<% write(getIndex("wlan_idx")); %>" value="">

<input type="hidden" id="wlan_wapi_psk_fmt" name="wapiPskFormat<% write(getIndex("wlan_idx")); %>" value="">
<input type="hidden" id="wlan_wapi_psk" name="wapiPskValue<% write(getIndex("wlan_idx")); %>" value="">
<input type="hidden" id="wlan_as_server_ip" name="wapiASIP<% write(getIndex("wlan_idx")); %>" value="">	
<input type="hidden" id="wlan_wapi_cert_sel" name="wapiCertSel<% write(getIndex("wlan_idx")); %>" value="">	
