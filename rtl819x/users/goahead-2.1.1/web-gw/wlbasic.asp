...
function LoadSetting()
 {
	...
	if(wmm_value==0)
 	...
 	{
 		if(!document.wlanSetup.elements["wlanDisabled"+<% write(getIndex("wlan_idx")); %>].checked)
...
<INPUT type=hidden name="regdomain<% write(getIndex("wlan_idx"));%>" value=<% write(getIndex("regDomain")); %>>
<INPUT type=hidden name="countrystr<% write(getIndex("wlan_idx"));%>" value=<% getInfo("country_str"); %>>
...
<table border=0 width="500" cellspacing=4>
 ...
 as well as wireless network parameters.
  ...
  <tr style="display:none">
      ...
      <td width="74%"><font size=2><select size="1" name="country<% write(getIndex("wlan_idx")); %>" onChange="updateCountry(document.wlanSetup,wlan_idx);updateBand(document.wlanSetup, wlan_idx);updateChan(document.wlanSetup, wlan_idx)">
      </select>
      </td>
  </tr>
  <tr>
      <td width="26%"><font size=2><b>Band:</b></td>
      <td width="74%"><font size=2><select size="1" name="band<% write(getIndex("wlan_idx")); %>" onChange="updateBand(document.wlanSetup, wlan_idx);Set_onChangeBand(document.wlanSetup, wlan_idx, document.wlanSetup.band<% write(getIndex("wlan_idx")); %>, document.wlanSetup.band<% write(getIndex("wlan_idx")); %>.selectedIndex);Set_SSIDbyBand(document.wlanSetup, wlan_idx,document.wlanSetup.band<% write(getIndex("wlan_idx")); %>, document.wlanSetup.band<% write(getIndex("wlan_idx")); %>.selectedIndex)">
