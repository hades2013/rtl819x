...
function change_1x_eap_type_settings(wlan_id)
{
	...
	index=document.formEncrypt.elements["eapType"+<% write(getIndex("wlan_idx")); %>].selectedIndex;
	if(index ==0){//eap-md5
		disableTextField(document.formEncrypt.elements["eapInsideType"+<% write(getIndex("wlan_idx")); %>]);	
		enableTextField(document.formEncrypt.elements["eapUserId"+<% write(getIndex("wlan_idx")); %>]);
		enableTextField(document.formEncrypt.elements["radiusUserName"+<% write(getIndex("wlan_idx")); %>]);
		enableTextField(document.formEncrypt.elements["radiusUserPass"+<% write(getIndex("wlan_idx")); %>]);
		disableTextField(document.formEncrypt.elements["radiusUserCertPass"+<% write(getIndex("wlan_idx")); %>]);
	}
	else if(index ==1){//eap-tls
		disableTextField(document.formEncrypt.elements["eapInsideType"+<% write(getIndex("wlan_idx")); %>]);	
		enableTextField(document.formEncrypt.elements["eapUserId"+<% write(getIndex("wlan_idx")); %>]);
		disableTextField(document.formEncrypt.elements["radiusUserName"+<% write(getIndex("wlan_idx")); %>]);
		disableTextField(document.formEncrypt.elements["radiusUserPass"+<% write(getIndex("wlan_idx")); %>]);
		enableTextField(document.formEncrypt.elements["radiusUserCertPass"+<% write(getIndex("wlan_idx")); %>]);
	}
	else if(index ==2){//eap-peap
		enableTextField(document.formEncrypt.elements["eapInsideType"+<% write(getIndex("wlan_idx")); %>]);
		enableTextField(document.formEncrypt.elements["eapUserId"+<% write(getIndex("wlan_idx")); %>]);
		
		insideIndex=document.formEncrypt.elements["eapInsideType"+<% write(getIndex("wlan_idx")); %>].selectedIndex;
...
//			get_by_id("show_pre_auth").style.display = "";		
		...
		if("<% getInfo('wlan_onoff_tkip'); %>" * 1 == 0)
		{
			...
			if(wlanMode!=1 && (wlanBand == 8 || wlanBand == 10 || wlanBand == 11 || wlanBand == 12) && allow_tkip==0)
			{
...
//      get_by_id("show_pre_auth").style.display = "none";
        ...
        if (dF.wapiAuth<% write(getIndex("wlan_idx")); %>[1].checked){
			...
			else
			{
				...
				enableTextField(dF.wapiASIP<% write(getIndex("wlan_idx")); %>);
				enableTextField(dF.wapiCertSel<% write(getIndex("wlan_idx")); %>);
			}
                }
		if (dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value == "<% getInfo("ip-rom"); %>")
		{
			if('<% getInfo("wapiLocalAsSupport");%>'=='true')
			{
				...
				dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex=2;	//0: use remote as0 cert; 1: use remote as1 cert; 2: use local as cert

				disableTextField(dF.wapiASIP<% write(getIndex("wlan_idx")); %>);
				disableTextField(dF.wapiCertSel<% write(getIndex("wlan_idx")); %>);
			}
			else
			{
				dF.uselocalAS<% write(getIndex("wlan_idx")); %>.checked=false;
...
//	alert("show_wapi_ASip: SSIDnum =" + SSIDnum);	//Added for test
	...
	{	
		if(SSIDnum==0){
			dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value = "<%getVirtualInfo("wapiASIp", 0);%>";
			dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 0);%>";
		}

		if(SSIDnum==1){
			dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value = "<%getVirtualInfo("wapiASIp", 1);%>";
			dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 1);%>";
		}

		if(SSIDnum==2){
			dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value = "<%getVirtualInfo("wapiASIp", 2);%>";
			dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 2);%>";
		}

		if(SSIDnum==3){
			dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value = "<%getVirtualInfo("wapiASIp", 3);%>";
			dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 3);%>";
		}

		if(SSIDnum==4){
			dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value = "<%getVirtualInfo("wapiASIp", 4);%>";
			dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 4);%>";
		}

		if(SSIDnum==5){
			dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value = "<%getVirtualInfo("wapiASIp", 5);%>";
			dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 5);%>";
		}
...
//	get_by_id("show_pre_auth").style.display = "none";
	...
	}else if (wep_type.selectedIndex > 1 && wep_type.selectedIndex < 4){
	...
	else if (wep_type.selectedIndex==4&&ap_mode!=1)
	{
		get_by_id("setting_wpa").style.display = "";
		get_by_id("show_wpa_cipher").style.display = "";
		get_by_id("show_wpa2_cipher").style.display = "";	
...
function Load_Setting()
{
	...
	<%	write ("\t\tmode=\"" + getVirtualIndex("encrypt", 0) + "\";\n");
		...
		write ("\t\twapi_auth=\"" + getVirtualIndex("wapiAuth", 0) + "\";\n");
		...
		write ("\t\twapi_psk_fmt=\"" + getVirtualIndex("wapiPskFormat", 0) + "\";\n");
		
		write ("\t\ttmp_eap_type=\"" + getVirtualIndex("eapType", 0) + "\";\n");
		write ("\t\ttmp_eap_inside_type=\"" + getVirtualIndex("eapInsideType", 0) + "\";\n");
	%>	
		dF.pskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("pskValue", 0);%>";	
		dF.wapiPskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiPskValue", 0);%>";	
		dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiASIp", 0);%>";
		dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 0);%>";
		...
		dF.eapUserId<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("eapUserId", 0);%>";
		dF.radiusUserName<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("radiusUserName", 0);%>";
		dF.radiusUserPass<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("radiusUserPass", 0);%>";
		dF.radiusUserCertPass<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("radiusUserCertPass", 0);%>";
	...
	<%	write ("\t\tmode=\"" + getVirtualIndex("encrypt", 1) + "\";\n");
		...
		write ("\t\twapi_auth=\"" + getVirtualIndex("wapiAuth", 1) + "\";\n");
		...
		write ("\t\twapi_psk_fmt=\"" + getVirtualIndex("wapiPskFormat", 1) + "\";\n");

		write ("\t\ttmp_eap_type=\"" + getVirtualIndex("eapType", 0) + "\";\n");
		write ("\t\ttmp_eap_inside_type=\"" + getVirtualIndex("eapInsideType", 0) + "\";\n");
	%>	
		dF.pskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("pskValue", 1);%>";	
		dF.wapiPskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiPskValue", 1);%>";	
		dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiASIp", 1);%>";
		dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 1);%>";
	...
	<%	write ("\t\tmode=\"" + getVirtualIndex("encrypt", 2) + "\";\n");
		...
		write ("\t\twapi_auth=\"" + getVirtualIndex("wapiAuth", 2) + "\";\n");
		...
		write ("\t\twapi_psk_fmt=\"" + getVirtualIndex("wapiPskFormat", 2) + "\";\n");

		write ("\t\ttmp_eap_type=\"" + getVirtualIndex("eapType", 0) + "\";\n");
		write ("\t\ttmp_eap_inside_type=\"" + getVirtualIndex("eapInsideType", 0) + "\";\n");
	%>	
		dF.pskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("pskValue", 2);%>";	
		dF.wapiPskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiPskValue", 2);%>";	
		dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiASIp", 2);%>";
		dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 2);%>";
	...
	<%	write ("\t\tmode=\"" + getVirtualIndex("encrypt", 3) + "\";\n");
		...
		write ("\t\twapi_auth=\"" + getVirtualIndex("wapiAuth", 3) + "\";\n");
		...
		write ("\t\twapi_psk_fmt=\"" + getVirtualIndex("wapiPskFormat", 3) + "\";\n");

		write ("\t\ttmp_eap_type=\"" + getVirtualIndex("eapType", 0) + "\";\n");
		write ("\t\ttmp_eap_inside_type=\"" + getVirtualIndex("eapInsideType", 0) + "\";\n");
	%>	
		dF.pskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("pskValue", 3);%>";	
		dF.wapiPskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiPskValue", 3);%>";	
		dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiASIp", 3);%>";
		dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 3);%>";
	...
	<%	write ("\t\tmode=\"" + getVirtualIndex("encrypt", 4) + "\";\n");
		...
		write ("\t\twapi_auth=\"" + getVirtualIndex("wapiAuth", 4) + "\";\n");
		...
		write ("\t\twapi_psk_fmt=\"" + getVirtualIndex("wapiPskFormat", 4) + "\";\n");

		write ("\t\ttmp_eap_type=\"" + getVirtualIndex("eapType", 0) + "\";\n");
		write ("\t\ttmp_eap_inside_type=\"" + getVirtualIndex("eapInsideType", 0) + "\";\n");
	%>	
		dF.pskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("pskValue", 4);%>";	
		dF.wapiPskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiPskValue", 4);%>";	
		dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiASIp", 4);%>";
		dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 4);%>";
	...
	<%	write ("\t\tmode=\"" + getVirtualIndex("encrypt", 5) + "\";\n");
		...
		write ("\t\twapi_auth=\"" + getVirtualIndex("wapiAuth", 5) + "\";\n");
		...
		write ("\t\twapi_psk_fmt=\"" + getVirtualIndex("wapiPskFormat", 5) + "\";\n");

		write ("\t\ttmp_eap_type=\"" + getVirtualIndex("eapType", 0) + "\";\n");
		write ("\t\ttmp_eap_inside_type=\"" + getVirtualIndex("eapInsideType", 0) + "\";\n");
	%>	
		dF.pskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("pskValue", 5);%>";	
		dF.wapiPskValue<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiPskValue", 5);%>";	
		dF.wapiASIP<% write(getIndex("wlan_idx")); %>.value="<%getVirtualInfo("wapiASIp", 5);%>";
		dF.wapiCertSel<% write(getIndex("wlan_idx")); %>.selectedIndex = "<%getVirtualInfo("wapiCertSel", 5);%>";
	...
	if(mode =='0')
		...
		dF.method<% write(getIndex("wlan_idx")); %>.selectedIndex=5;
	else if (mode =='7' && ap_mode == 1)	
		dF.method<% write(getIndex("wlan_idx")); %>.selectedIndex=4;
	else 
		dF.method<% write(getIndex("wlan_idx")); %>.selectedIndex=0;
	...
	if(tmp_eap_type=='0'){
		dF.eapType<% write(getIndex("wlan_idx")); %>.selectedIndex=0;
	}
	else if(tmp_eap_type=='1'){
		dF.eapType<% write(getIndex("wlan_idx")); %>.selectedIndex=1;
	}
	else if(tmp_eap_type=='2'){
		dF.eapType<% write(getIndex("wlan_idx")); %>.selectedIndex=2;
	}
	else{
	...
	if(tmp_eap_inside_type=='0'){
		dF.eapInsideType<% write(getIndex("wlan_idx")); %>.selectedIndex=0;
	}
	else{
...
//		dF.preAuth<% write(getIndex("wlan_idx")); %>.checked=true;
	if (wapi_auth == 1)
		dF.wapiAuth<% write(getIndex("wlan_idx")); %>[0].checked=true;
	else
		dF.wapiAuth<% write(getIndex("wlan_idx")); %>[1].checked=true;

	if (wapi_psk_fmt ==0)
		dF.wapiPskFormat<% write(getIndex("wlan_idx")); %>.selectedIndex=0;
	else		
		dF.wapiPskFormat<% write(getIndex("wlan_idx")); %>.selectedIndex=1;
...
function ValidateForm(passForm)
{
	...
	if((enc_mode == 7))
	{
		if((tF.wapiAuth<% write(getIndex("wlan_idx")); %>[1].checked))
	{
		//add some check for WAPI
			if (check_wapikey(tF, <% write(getIndex("wlan_idx")); %>) != true)
				return false;
		}
	}
	else {
...
//	if(client_mode_support_1x == 1)
	{
		passForm.eapType<% write(getIndex("wlan_idx")); %>.value = tF.eapType<% write(getIndex("wlan_idx")); %>.value;
		passForm.eapInsideType<% write(getIndex("wlan_idx")); %>.value = tF.eapInsideType<% write(getIndex("wlan_idx")); %>.value;
		passForm.eapUserId<% write(getIndex("wlan_idx")); %>.value = tF.eapUserId<% write(getIndex("wlan_idx")); %>.value;
		passForm.radiusUserName<% write(getIndex("wlan_idx")); %>.value = tF.radiusUserName<% write(getIndex("wlan_idx")); %>.value;
		passForm.radiusUserPass<% write(getIndex("wlan_idx")); %>.value = tF.radiusUserPass<% write(getIndex("wlan_idx")); %>.value;
		passForm.radiusUserCertPass<% write(getIndex("wlan_idx")); %>.value = tF.radiusUserCertPass<% write(getIndex("wlan_idx")); %>.value;
	}
...
//		passForm.preAuth<% write(getIndex("wlan_idx")); %>.value = "OFF";		
	...
	passForm.wapiAuth<% write(getIndex("wlan_idx")); %>[0].checked = tF.wapiAuth<% write(getIndex("wlan_idx")); %>[0].checked;
	passForm.wapiAuth<% write(getIndex("wlan_idx")); %>[1].checked = tF.wapiAuth<% write(getIndex("wlan_idx")); %>[1].checked;	
	passForm.wapiPskFormat<% write(getIndex("wlan_idx")); %>.value= tF.wapiPskFormat<% write(getIndex("wlan_idx")); %>.value;
	passForm.wapiPskValue<% write(getIndex("wlan_idx")); %>.value = tF.wapiPskValue<% write(getIndex("wlan_idx")); %>.value;
	passForm.wapiASIP<% write(getIndex("wlan_idx")); %>.value = tF.wapiASIP<% write(getIndex("wlan_idx")); %>.value;
	passForm.wapiCertSel<% write(getIndex("wlan_idx")); %>.value = tF.wapiCertSel<% write(getIndex("wlan_idx")); %>.value;
        ...
        <tr id="setting_wapi" style="display:none">
				...
				<td width="30%" bgcolor="#FFFFFF" class="bgblue"><font size="2"><b>Pre-Shared&nbsp;Key&nbsp;Format:</b></font></td>
                                ...
                                <tr id="show_wapi_psk2" style="display:none">
                                        <td width="30%" bgcolor="#FFFFFF" class="bgblue"><font size="2"><b>Pre-Shared&nbsp;Key:</b></font></td>
                                        <td width="70%" class="bggrey"><input type="password" name="wapiPskValue<% write(getIndex("wlan_idx")); %>" id="wapipsk" size="32" maxlength="64" value="">
                                </td></tr>
                        </table>
	</td></tr>
	...
	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="4">
			<tr>
				 ...
				 <td width="70%" class="bggrey"><font size="2">
				 <select size="1"  id="eap_type" name="eapType<% write(getIndex("wlan_idx")); %>" onChange="change_1x_eap_type_settings(wlan_idx)">	 
			...
			<tr>
				 ...
				 <td width="70%" class="bggrey"><font size="2">
				 <select size="1"  id="eap_inside_type" name="eapInsideType<% write(getIndex("wlan_idx")); %>" onChange="change_1x_eap_type_settings(wlan_idx)">	 
		 			<option value="0"> MSCHAPV2 </option>
				 </select> </font></td>
			</tr>
			<tr>
				 <td width="30%" class="bgblue"><font size="2"><b>EAP User ID:</b></font></td>
				 <td width="70%" class="bggrey"><input id="eap_user_id" name="eapUserId<% write(getIndex("wlan_idx")); %>" size="24" maxlength="64" value=""></td>
			</tr>
			<tr>
				 <td width="30%" class="bgblue"><font size="2"><b>RADIUS User Name:</b></font></td>
				 <td width="70%" class="bggrey"><input id="radius_user_name" name="radiusUserName<% write(getIndex("wlan_idx")); %>" size="24" maxlength="64" value=""></td>
			</tr>
			<tr>
				<td width="30%" class="bgblue"><font size="2"><b>RADIUS User Password:</b></font></td>
				<td width="70%" class="bggrey"><input type="password" id="radius_user_pass" name="radiusUserPass<% write(getIndex("wlan_idx")); %>" size="24" maxlength="64" value=""></td>
			</tr>
			<tr>
				<td width="30%" class="bgblue"><font size="2"><b>User Key Password (if any):</b></font></td>
				<td width="70%" class="bggrey"><input type="password" id="radius_user_cert_pass" name="radiusUserCertPass<% write(getIndex("wlan_idx")); %>" size="24" maxlength="64" value=""></td>
			</tr>
		</table>								
	</td>
	</tr>
        ...
        <tr id="show_8021x_wapi" style="display:none">
				...
				<tr>
                                         ...
                                         <td width="70%" class="bggrey"><input id="wapiAS_ip" name="wapiASIP<% write(getIndex("wlan_idx")); %>" size="16" maxlength="15" value="0.0.0.0"></td></tr>
				<tr>
					 <td width="30%" class="bgblue"><font size="2"><b>Select WAPI certificate:</b></font></td>					 
				  	<td width="70%" class="bggrey"><font size="2">
					 <select size="1"  id="wapi_cert_sel" name="wapiCertSel<% write(getIndex("wlan_idx")); %>" onChange="show_wapi_cert_sel()">	 
