...
#ifdef VOIP_SUPPORT
	if( websVoipAccess( wp ) )
	{
		printf( "Local access!(%s)\n", wp->ipaddr );
	} else
#endif
	if (am == AM_NONE) {
...
/******************************************************************************/
/*
 *	VoIP application access 
 */
#ifdef VOIP_SUPPORT
int websVoipAccess( webs_t wp )
{
	...
	if( wp ->userAgent == NULL || 
	    ( gstrcmp( wp ->userAgent, T( "RTK-IVR" ) ) &&
	      gstrcmp( wp ->userAgent, T( "RTK-FW" ) ) ) )
	{
		return 0;
	}

	// check access ip 
	if( websGetRequestFlags( wp ) & WEBS_LOCAL_REQUEST )
		return 1;

	// check cookie 
	if( ( wp->flags & WEBS_COOKIE ) == 0 || 
	    ( wp ->cookie == NULL ) || 
	    ( p = strstr( wp ->cookie, "voip-sid=" ) ) == NULL )
	{
		return 0;
	}
