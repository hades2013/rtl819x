...
int setWlan_Applications(char *action, char *argv)
{
	...
	int wlan_iapp_disabled_root=0,wlan_wsc_disabled_root=0, wlan_network_type_root=0, wlan_wsc_disabled_vxd=0;
...
//for WPS
	if (isFileExist("/bin/wscd")) {
		...
		apmib_get( MIB_WLAN_WSC_DISABLE, (void *)&wlan_wsc_disabled_vxd);
...
//fprintf(stderr,"\r\n WSC_UPNP_Enabled=[%d],__[%s-%u]",WSC_UPNP_Enabled,__FILE__,__LINE__);						
					...
					else if(!strcmp(token, "wlan0-vxd") && (wlan_wsc_disabled_vxd != 0))
					{
							WSC=0;
					}
					else{
