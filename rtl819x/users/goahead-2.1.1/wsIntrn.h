...
#define FILE_UPLOAD_WEBS_TIMEOUT		10000		/* General request timeout (6) */
#define PAGE_READ_BUFSIZE	0x10000			/* bytes read from page files */
//#define PAGE_READ_BUFSIZE	(65535*2)			/* bytes read from page files */
#else
...
extern int 		 websDefaultHandler(webs_t wp, char_t *urlPrefix, 
					...
					char_t *query);
extern int websPathHandler(webs_t wp, char_t *urlPrefix, 
					char_t *webDir, int arg, char_t *url, char_t *path, 
					char_t *query);
...
extern int		 websPageIsFile(char_t *lpath);
